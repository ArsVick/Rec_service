...
def pred_fun(user_id: int):
    set_id = set(books_full_an.id.tolist())
    list_of_books = circulaton_full.loc[circulaton_full['readerID'] == user_id].catalogueRecordID.tolist()
...
def get_history_list(user_id: int):
    list_of_books = circulaton_full.loc[circulaton_full['readerID'] == user_id].catalogueRecordID.tolist()
    return list_of_books


def get_full_information_by_id(book_id : int) :
    tmp = books.loc[books['id'] == int(book_id)]
    try:
        autor = list(tmp.author_fullName)[0]
    except IndexError as e:
        autor = None

    try:
        book_name  = list(tmp.title_orig)[0]
    except IndexError as e:
        book_name = None
    return book_name , autor
# get_full_information_by_id(1054736)
